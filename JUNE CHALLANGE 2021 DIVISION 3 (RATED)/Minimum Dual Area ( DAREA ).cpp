...
ll solve(vector<vector<ll>>&P, int n)
{
	unordered_map<ll, ll> min_X, max_X, min_Y, max_Y;
  	...
  	for(auto vec: P)
  	{
    	...
    	min_X[y] = min(min_X[y], x); max_X[y] = max(max_X[y], x);
    	min_Y[x] = min(min_Y[x], y); max_Y[x] = max(max_Y[x], y);
  	}
  	vector<ll> X, Y;
  	for(auto pi: min_Y) X.push_back(pi.first);
  	for(auto pi: min_X) Y.push_back(pi.first);
  	...
  	for(auto x: X)
	{
    	mn = min(mn, min_Y[x]); mx = max(mx, max_Y[x]);
